...
// Validate file name (for our purposes we'll just remove invalid characters)
	...
	$file_name = md5($file_name . $_SERVER['REMOTE_PORT'] . time()) .".". $file_ext ;
        $file_name_thumb = md5($file_name . $_SERVER['REMOTE_PORT'] . time()) ."_thumb.". $file_ext ;
	...
	*/
	if (!move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
		...
		exit(0);
            ...
            $imagem->redimensiona('640','480','');
            $imagem->grava($save_path.$file_name);  
            $imagem->redimensiona('160','120','');
            $imagem->grava($save_path.$file_name_thumb);            
        }
