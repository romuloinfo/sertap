...
// Settings
  ...
  //          $path .= $dir[$x]."\\";
   ...
   //    $save_path  = $path."img_pub\\";
	...
	$extension_whitelist = array("gif","jpeg", "jpg","png");	// Allowed file extensions
...
// Validate file name (for our purposes we'll just remove invalid characters)
	...
	$file_name = md5($file_name . $_SERVER['REMOTE_PORT'] . time()) .".".$file_ext ;
        $file_name_thumb = $file_name."_thumb.".$file_ext ;
	...
	*/
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
		...
		exit(0);
            ...
            $foto->setImagem($file_name);
            $foto->setImagem_thub($file_name_thumb);
            $foto->setId_imovel($imovel);
            $foto->salvar();
            
            $imagem = new m2brimagem($save_path.$file_name);                        
            $imagem->redimensiona('640','480','crop');
            $imagem->grava($save_path.$file_name);  
            
            $imagem2 = new m2brimagem($save_path.$file_name);             
            $imagem2->redimensiona('160','120','crop');
            $imagem2->grava($save_path.$file_name_thumb);            
        }
